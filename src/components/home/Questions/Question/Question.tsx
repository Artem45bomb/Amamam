...
export const Question: FC<Props> = ({title, text}) => {
    ...
    return <div className={'w-full border-b border-secondary pb-5 sm:pb-10 font-gilroy'}>
        <div className={cn('flex items-center justify-between transition-all', active ? "mb-12" : "")}>
            <h2 className={'font-semibold text-xl sm:text-3xl text-black'}>{title}</h2>
        ...
        <p className={cn("font-normal text-black text-[1.1rem] transition-all", active ? "" : "opacity-0 h-0 text-[0rem]")}>{text}</p>
    </div>
